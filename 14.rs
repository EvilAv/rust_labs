...
impl User {
    ...
    pub fn visit_doctor(&mut self, measurements: Measurements) -> HealthReport {
        ...
        // Вычисляем изменение давления (если есть предыдущее значение)
        let blood_pressure_change = match self.last_blood_pressure {
            Some((prev_systolic, prev_diastolic)) => {
                Some((
                    measurements.blood_pressure.0 as i32 - prev_systolic as i32,
                    measurements.blood_pressure.1 as i32 - prev_diastolic as i32,
                ))
            }
            None => None,
        };
